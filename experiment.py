...
def simulate_experiment(df, feature_name, launch_date="2017-01-01", seed=42):
    ...
    feature_effects = {
        "AI Comment Suggestions": {"session_boost": 0.08, "conversion_boost": 0.002, "quality_boost": 0.10},
        "AI Generated Captions": {"session_boost": 0.15, "conversion_boost": 0.005, "quality_boost": 0.20},
        "AI Search Assistant": {"session_boost": 0.25, "conversion_boost": 0.010, "quality_boost": 0.30},
        "AI Recommendations": {"session_boost": -0.05, "conversion_boost": -0.001, "quality_boost": -0.08}
    }

    effects = feature_effects.get(feature_name, {"session_boost": 0.05, "conversion_boost": 0.001, "quality_boost": 0.05})

    unique_users = df["user_id"].unique()
    treatment_users = set(np.random.choice(unique_users, size=len(unique_users) // 2, replace=False))

    df = df.copy()
    df["feature"] = feature_name
    df["group"] = df["user_id"].apply(lambda uid: "treatment" if uid in treatment_users else "control")

    launch_date = pd.Timestamp(launch_date)
    df["period"] = df["session_date"].apply(lambda d: "post" if d >= launch_date else "pre")
    df["weeks_since_launch"] = ((df["session_date"] - launch_date).dt.days // 7)
